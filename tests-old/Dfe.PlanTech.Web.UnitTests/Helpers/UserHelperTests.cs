...
public class UserHelperTests
{
    ...
    {
        new UserEntity() { DfeSignInRef = FIRST_USER_REF, Id = 1 },
        new UserEntity() { DfeSignInRef = SECOND_USER_REF, Id = 2 },
    };
    ...
    public UserHelperTests()
    {
        ...
        _httpContextAccessorSubstitute.HttpContext.Returns(
            new DefaultHttpContext() { User = user }
        );

        _userHelper = new UserHelper(
            _httpContextAccessorSubstitute,
            _planTechDbContextSubstitute,
            _createEstablishmentCommandSubstitute,
            _getUserIdQuerySubstitute,
            _getEstablishmentIdQuerySubstitute
        );
    }
    ...
    {
        _getEstablishmentIdQuerySubstitute
            .GetEstablishmentId(Arg.Any<string>())
            .Returns(
                (callInfo) =>
                {
                    ...
                    return _establishments
                        .Where(establishment => establishment.EstablishmentRef == establishmentRef)
                        .Select(establishment => establishment.Id)
                        .FirstOrDefault();
                }
            );
        _getEstablishmentIdQuerySubstitute
            .GetGroupEstablishments(Arg.Any<int>())
            .Returns(
                (callInfo) =>
                {
                    var establishmentId = callInfo.ArgAt<int>(0);
                    return _groupEstablishments
                        .Where(link => link.GroupUid == GROUP_ESTABLISHMENT_REF)
                        .ToList();
                }
            );
    }
        ...
        _getUserIdQuerySubstitute
            ...
            .Returns(
                (callInfo) =>
                {
                    ...
                    return _users
                        .Where(user => user.DfeSignInRef == userRef)
                        .Select(user => user.Id)
                        .FirstOrDefault();
                }
            );
    ...
    public async Task GetEstablishmentId_Returns_Correct_Id_When_Establishment_Exists_In_DB()
    {
        ...
        await _createEstablishmentCommandSubstitute
            .Received(0)
            .CreateEstablishment(Arg.Any<EstablishmentDto>());
    ...
    public async Task GetEstablishmentId_Throws_Exception_When_Reference_Is_Not_Present()
    {
        ...
        var userHelperWithMissingOrgData = new UserHelper(
            _httpContextAccessorSubstitute,
            _planTechDbContextSubstitute,
            _createEstablishmentCommandSubstitute,
            _getUserIdQuerySubstitute,
        ...
        await Assert.ThrowsAnyAsync<Exception>(() =>
            userHelperWithMissingOrgData.GetEstablishmentId()
        );
    }
    ...
    {
        var groupUser = new ClaimsPrincipal(
            new ClaimsIdentity(
                new Claim[]
                {
        ...
        _httpContextAccessorSubstitute.HttpContext.Returns(
            new DefaultHttpContext() { User = groupUser }
        );

        var groupUserHelper = new UserHelper(
            _httpContextAccessorSubstitute,
            _planTechDbContextSubstitute,
            _createEstablishmentCommandSubstitute,
            _getUserIdQuerySubstitute,
    ...
    public async Task SetEstablishment_Creates_Establishment_And_Returns_Id()
    {
        ...
        await _createEstablishmentCommandSubstitute
            .Received(1)
            .CreateEstablishment(
                Arg.Is<EstablishmentDto>(dto => dto.Reference == establishmentDto.Reference)
            );
    }
}
