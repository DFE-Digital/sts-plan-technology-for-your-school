...
namespace Dfe.PlanTech.Web.UnitTests.Controllers
{
    public class CheckAnswersControllerTests
    {
        ...
        public CheckAnswersControllerTests()
        {
            ...
            _userJourneyMissingContentExceptionHandler =
            ...
            _getRecommendationRouter
                .GetRecommendationSlugForSection(_sectionSlug, Arg.Any<CancellationToken>())
        ...
        [InlineData("")]
        [InlineData(null)]
        public async Task CheckAnswersPage_Should_ThrowException_When_SectionSlug_NullOrEmpty(
            string? section
        )
        {
            await Assert.ThrowsAnyAsync<ArgumentException>(() =>
                _checkAnswersController.CheckAnswersPage(
                    section!,
                    false,
                    _checkAnswersRouter,
                    _userJourneyMissingContentExceptionHandler,
        ...
        {
            await _checkAnswersController.CheckAnswersPage(
                _sectionSlug,
                false,
                _checkAnswersRouter,
                _userJourneyMissingContentExceptionHandler,
            ...
            await _checkAnswersRouter
                .Received()
                .ValidateRoute(
                    _sectionSlug,
                    null,
                    _checkAnswersController,
        ...
        public async Task CheckAnswersPage_Should_Call_UserJourneyMissingContentExceptionHandler_When_Exception_Thrown()
        {
            ...
            await _userJourneyMissingContentExceptionHandler
                .Received()
                .Handle(_checkAnswersController, exception, Arg.Any<CancellationToken>());
        }
        ...
        [InlineData(-1)]
        [InlineData(-100)]
        public async Task ConfirmAnswers_Should_ThrowException_When_SubmissionId_OutOfRange(
            int submissionId
        )
        {
            await Assert.ThrowsAnyAsync<ArgumentOutOfRangeException>(() =>
                _checkAnswersController.ConfirmCheckAnswers(
                    _sectionSlug,
                    submissionId,
                    "section name",
                    _redirectOption,
                    _calculateMaturityCommand,
                    _getRecommendationRouter,
                    _markSubmissionAsReviewedCommand
                )
            );
        }
        ...
        [InlineData(null)]
        [InlineData("")]
        public async Task ConfirmAnswers_Should_ThrowException_When_SectionName_NullOrEmpty(
            string? sectionName
        )
        {
            await Assert.ThrowsAnyAsync<ArgumentException>(() =>
        ...
        public async Task ConfirmAnswers_Should_CalculateMaturity_When_ArgsValid()
        {
            ...
            _calculateMaturityCommand
                .CalculateMaturityAsync(Arg.Any<int>(), Arg.Any<CancellationToken>())
                .Returns(
                    (callinfo) =>
                    {
            ...
            var result = await _checkAnswersController.ConfirmCheckAnswers(
                _sectionSlug,
                submissionId,
                "section name",
                _redirectOption,
        ...
        public async Task ConfirmAnswers_Should_Redirect_To_SelfAssessmentPage()
        {
            ...
            Assert.True(
                redirectToActionResult.RouteValues["route"] is string s
                    && s == UrlConstants.SelfAssessmentPage
            );
        }
        ...
        public async Task ConfirmAnswers_Should_Redirect_To_Recommendations()
        {
            ...
            Assert.True(
                redirectToActionResult.RouteValues["sectionSlug"] is string slug
                    && slug == _sectionSlug
            );
            Assert.True(redirectToActionResult.RouteValues.ContainsKey("recommendationSlug"));
            Assert.True(
                redirectToActionResult.RouteValues["recommendationSlug"] is string recSlug
                    && recSlug == _recommendationSlug
            );
        }
        ...
        public async Task ConfirmAnswers_Should_Redirect_To_CheckAnswers()
        {
            ...
            Assert.Equal(
                CheckAnswersController.ControllerName,
                redirectToActionResult.ControllerName
            );
            Assert.Equal(
                CheckAnswersController.CheckAnswersAction,
                redirectToActionResult.ActionName
            );
            Assert.NotNull(redirectToActionResult.RouteValues);
            Debug.Assert(
                redirectToActionResult.RouteValues != null,
                "checkAnswerResult.RouteValues != null"
            );
            ...
            Assert.Equal(
                ...
                _checkAnswersController.TempData["ErrorMessage"]
            );
        }
    }
}
