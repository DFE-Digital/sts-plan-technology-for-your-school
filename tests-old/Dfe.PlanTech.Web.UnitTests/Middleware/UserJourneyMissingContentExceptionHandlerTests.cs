...
{
    private readonly ILogger<UserJourneyMissingContentExceptionHandler> _logger = Substitute.For<
        ILogger<UserJourneyMissingContentExceptionHandler>
    >();
    private readonly IDeleteCurrentSubmissionCommand _deleteCurrentSubmissionCommand =
        Substitute.For<IDeleteCurrentSubmissionCommand>();
    ...
    public UserJourneyMissingContentExceptionHandlerTests()
    {
        ...
        {
            { UserJourneyMissingContentExceptionHandler.ErrorMessageConfigKey, ErrorMessage },
        };

        _configuration = new ConfigurationBuilder().AddInMemoryCollection(inMemorySettings).Build();

        _controller = new MockController() { TempData = tempData };
    }
    ...
    public async Task Handle_LogsError_DeletesSubmission_StoresErrorMessageAndRedirects()
    {
        ...
        {
            Name = "Section Name",
            Sys = new() { Id = "SectionId" },
        };

        var expectedLogMessage = $"Handling errored user journey for section {section.Name}";

        var errorHandler = new UserJourneyMissingContentExceptionHandler(
            _logger,
        ...
        await _deleteCurrentSubmissionCommand
            .Received(1)
    ...
    private void TestLoggedMessages(string message, LogLevel logLevel, int receivedCount)
    {
        ...
        var matchingCallCount = receivedCalls
            .Where(args => args.message == message && args.logLevel == logLevel)
            .Count();
        if (receivedCalls.Length != receivedCount)
        {
            ...
            Assert.Fail(
                $"Expected {receivedCount} logging messages to match but found {receivedCalls.Length} matching calls. Received calls: \n"
                    + actualReceivedCalls
            );
        }
    }
