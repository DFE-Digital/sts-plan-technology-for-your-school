...
public class GetSubTopicRecommendationQueryTests
{
    ...
    private readonly ILogger<GetSubTopicRecommendationQuery> _logger = Substitute.For<
        ILogger<GetSubTopicRecommendationQuery>
    >();
    ...
    public GetSubTopicRecommendationQueryTests()
    {
        ...
        {
            Answers = new List<Answer>() { new Answer(), new Answer(), new Answer() },
            Chunks = new List<RecommendationChunk>()
            {
                new RecommendationChunk()
                {
                    ...
                    {
                        new Answer() { Sys = new SystemDetails() { Id = "1" } },
                        new Answer() { Sys = new SystemDetails() { Id = "2" } },
                        new Answer() { Sys = new SystemDetails() { Id = "3" } },
                    },
                    Header = "chunk1",
                },
                new RecommendationChunk()
                {
                    Answers = new List<Answer>()
                    {
                        new Answer() { Sys = new SystemDetails() { Id = "4" } },
                        new Answer() { Sys = new SystemDetails() { Id = "5" } },
                        new Answer() { Sys = new SystemDetails() { Id = "6" } },
                    },
                    Header = "chunk3",
                },
                new RecommendationChunk()
                {
                    Answers = new List<Answer>()
                    {
                        new Answer() { Sys = new SystemDetails() { Id = "7" } },
                        new Answer() { Sys = new SystemDetails() { Id = "8" } },
                        new Answer() { Sys = new SystemDetails() { Id = "9" } },
                    },
                    Header = "chunk2",
                },
            },
        ...
        _subTopicOne = new() { Sys = new SystemDetails() { Id = "SubTopicId" } };

        _subTopicTwo = new() { Sys = new SystemDetails() { Id = "IdForAnotherSubTopic" } };
        ...
        _subtopicRecommendationOne = new()
        {
            ...
            [
                ...
                {
                    Maturity = "Low",
                    Header = new Header() { Text = "Low-Maturity-Intro" },
                },
                new RecommendationIntro()
                {
                    Maturity = "Medium",
                    Header = new Header() { Text = "Medium-Maturity-Intro" },
                },
                new RecommendationIntro()
                {
                    Maturity = "High",
                    Header = new Header() { Text = "High-Maturity-Intro" },
                },
            ],
        ...
        _subtopicRecommendationTwo = new() { Subtopic = _subTopicTwo };
    ...
    {
        _repoSubstitute
            .GetEntities<SubtopicRecommendation?>(
                Arg.Any<GetEntitiesOptions>(),
                Arg.Any<CancellationToken>()
            )
            .Returns([_subtopicRecommendationOne]);

        SubtopicRecommendation? subTopicRecommendation = await _query.GetSubTopicRecommendation(
            _subTopicOne.Sys.Id
        );
    ...
    public async Task GetSubTopicRecommendation_Returns_Correct_SubTopicRecommendation_From_SectionTwo()
    {
        _repoSubstitute
            ...
            .Returns([_subtopicRecommendationTwo]);
    ...
    public async Task GetSubTopicRecommendationChunk_When_List_Of_Answers_Passed_To_RecommendationSection_From_SectionOne()
    {
        ...
        var chunks = subTopicRecommendation!.Section.GetRecommendationChunksByAnswerIds(
    ...
    public async Task GetRecommendationsViewDto_Retrieves_Correct_Information_When_Existing()
    {
        _repoSubstitute
            ...
            .Returns(
                (callinfo) =>
                {
                    ...
                    var idFilter = options
                        .Queries!.Where(query => query.Field == "fields.subtopic.sys.id")
                        .Select(query => query as ContentQuerySingleValue)
                        .Select(query => query!.Value)
                        .FirstOrDefault();

                    return _subtopicRecommendations
                        .Where(rec => rec.Subtopic.Sys.Id == idFilter)
        ...
        var maturity = "Low";
        var expectedIntro = _subtopicRecommendationOne!.Intros.FirstOrDefault(intro =>
            intro.Maturity == maturity
        );

        Assert.NotNull(expectedIntro);

        var result = await _query.GetRecommendationsViewDto(
            _subtopicRecommendationOne!.Subtopic.Sys.Id,
            maturity
        );
    ...
    public async Task GetRecommendationsViewDto_Returns_Null_When_No_Match()
    {
        ...
        var result = await _query.GetRecommendationsViewDto("any id", "Low");

        Assert.Null(result);

        Assert.Single(
            _logger.GetMatchingReceivedMessages(
                "Could not find subtopic recommendation in Contentful for any id",
                LogLevel.Error
            )
        );
    }
