...
public class GetEntityFromContentfulQueryTests
{
    ...
    private readonly ILogger<GetEntityFromContentfulQuery> _logger = Substitute.For<
        ILogger<GetEntityFromContentfulQuery>
    >();
    private readonly GetEntityFromContentfulQuery _getEntityFromContentfulQuery;

    private readonly Question _firstQuestion = new()
    {
    ...
    {
        _getEntityFromContentfulQuery = new GetEntityFromContentfulQuery(
            _logger,
            _contentRepository
        );
    }
    ...
    {
        _contentRepository
            .GetEntityById<Question>(Arg.Any<string>(), cancellationToken: CancellationToken.None)
            .Throws(_ => new Exception("Contentful error"));

        var result = await _getEntityFromContentfulQuery.GetEntityById<Question>(
            _firstQuestion.Sys.Id
        );
        var receivedLoggerMessages = _logger.GetMatchingReceivedMessages(
            GetEntityFromContentfulQuery.ExceptionMessageEntityContentful,
            LogLevel.Error
        );
