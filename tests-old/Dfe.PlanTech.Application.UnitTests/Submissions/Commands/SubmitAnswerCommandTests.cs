...
public class SubmitAnswerCommandTests
{
    ...
    public SubmitAnswerCommandTests()
    {
        ...
        {
            Maturity = "Low",
            Answer = new IdWithText() { Id = "Id", Text = "Text" },
        };

        _dto = new SubmitAnswerDto() { ChosenAnswerJson = JsonSerializer.Serialize(_chosenAnswer) };
    }
    ...
    public async Task Should_Throw_Exception_If_Dto_Null()
    {
        await Assert.ThrowsAnyAsync<InvalidDataException>(() =>
            _submitAnswerCommand.SubmitAnswer(null!)
        );
    }
    ...
    public async Task Should_Throw_Exception_If_ChosenAnswer_Null()
    {
        ...
        await Assert.ThrowsAnyAsync<InvalidDataException>(() =>
            _submitAnswerCommand.SubmitAnswer(dto)
        );
    }
    ...
    public async Task Should_Throw_Exception_If_UserId_Null()
    {
        ...
        await Assert.ThrowsAnyAsync<AuthenticationException>(() =>
            _submitAnswerCommand.SubmitAnswer(_dto)
        );
    }
        ...
        _db.ExecuteSqlAsync(Arg.Any<FormattableString>(), Arg.Any<CancellationToken>())
            .Returns(
                (callInfo) =>
                {
                    ...
                    SqlParameter? responseParam = formattableString
                        .GetArguments()
                        .Select(argument =>
                            argument is SqlParameter sqlParameter ? sqlParameter : null
                        )
