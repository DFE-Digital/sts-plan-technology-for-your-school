...
{
    private readonly ILogger<ContentfulRepository> _logger = Substitute.For<ILogger<ContentfulRepository>>();
    ...
    public ContentfulRepositoryTests()
    {
        ...
        _clientSubstitute.GetEntries(Arg.Any<QueryBuilder<OtherTestClass>>(), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(new ContentfulCollection<OtherTestClass>
            {
        ...
        _automatedTestingOptions.Value.Returns(new AutomatedTestingOptions
        {
            Contentful = new()
            {
                IncludeTaggedContent = false,
                Tag = "e2e"
            }
        });
    }
        ...
        _logger.Received(1).Log(
            ...
            Arg.Is<IReadOnlyList<KeyValuePair<string, object>>>(state =>
                state.Any(kv => kv.Key == "{OriginalFormat}" &&
                                (string)kv.Value == "Error retrieving one or more {EntryType} entries from Contentful:\n{Errors}")
                && state.Any(kv => kv.Key == "EntryType" &&
                                    (string)kv.Value == nameof(OtherTestClass))
                && state.Any(kv => kv.Key == "Errors" &&
    ...
    {
        var repository = new ContentfulRepository(_logger, _clientSubstitute, _hostEnvironmentMock, _automatedTestingOptions);
        var result = await repository.GetPaginatedEntriesAsync<OtherTestClass>(new GetEntriesOptions());
        Assert.NotNull(result);
        Assert.Empty(result);
    }
    ...
    public async Task GetEntitiesCount_ReturnsCorrectCount()
    {
        ...
        _clientSubstitute.GetEntries(Arg.Any<QueryBuilder<RecommendationChunkEntry>>(), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(new ContentfulCollection<RecommendationChunkEntry> { Items = new List<RecommendationChunkEntry>() { new RecommendationChunkEntry() }, Total = 42, Errors = new List<ContentfulError>() }));
    ...
    {
        var repository = new ContentfulRepository(_logger, _clientSubstitute, _hostEnvironmentMock, _automatedTestingOptions);
        await Assert.ThrowsAsync<ArgumentNullException>(() => repository.GetEntryByIdAsync<TestClass>(""));
    }
    ...
    public async Task GetEntityById_Should_Throw_GetEntitiesIDException_When_DuplicateIds()
    {
        ...
        var repository = new ContentfulRepository(_logger, _clientSubstitute, _hostEnvironmentMock, _automatedTestingOptions);
        await Assert.ThrowsAsync<GetEntriesException>(() => repository.GetEntryByIdAsync<TestClass>(testId));
    }
    ...
    public async Task GetEntityById_Should_Throw_GetEntitiesIDException_With_Correct_Exception_Message_When_DuplicateIds()
    {
        var testId = "duplicateId";
        var repository = new ContentfulRepository(_logger, _clientSubstitute, _hostEnvironmentMock, _automatedTestingOptions);
        var exception = await Assert.ThrowsAsync<GetEntriesException>(() => repository.GetEntryByIdAsync<TestClass>(testId));
