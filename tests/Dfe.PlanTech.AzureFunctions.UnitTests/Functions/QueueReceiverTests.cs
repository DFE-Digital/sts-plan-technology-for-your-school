...
public class QueueReceiverTests
{
    ...
    private readonly List<QuestionDbEntity> _questions = new() { _otherContentComponent };
    private readonly List<AnswerDbEntity> _answers = new();

    private readonly List<ContentComponentDbEntity> _contentComponents = new();
    ...
    private void MockEntityType()
    {
        ...
        HashSet<string> nonNullablePropertyNames = new() { "Text", "Slug", "Answers" };
    ...
    public async Task QueueReceiverDbWriter_Should_ExitEarly_When_Event_Is_Save_And_Entity_Is_Published_And_UsePreview_Is_False(string subject)
    {
        ...
        await queueReceiver.QueueReceiverDbWriter(new[] { serviceBusReceivedMessage }, serviceBusMessageActionsMock, CancellationToken.None);
    ...
    public async Task QueueReceiverDbWriter_Should_DeadLetterQueue_If_Delete_Or_Unpublish_Missing_Entity(string subject)
    {
        ServiceBusReceivedMessage serviceBusReceivedMessageMock = Substitute.For<ServiceBusReceivedMessage>();
        ServiceBusMessageActions serviceBusMessageActionsMock = Substitute.For<ServiceBusMessageActions>();
