...
export const validateAndTestNonAuthorisedPages = (dataMapper) => {
  Array.from(dataMapper?.pages ?? [])
    .map(([, page]) => page)
    .filter(
      (page) => !page.fields.requiresAuthorisation && page.fields.internalName !== startPageName,
    )
    .forEach((page) => {
      it(
        'Should have correct content on non-authorised pages. Testing ' + page.fields.internalName,
        () => {
