const navigateToRecommendationPage = () => {
    ...
    return cy.navigateToCheckAnswersPage(selectedQuestionsWithAnswers).then((res) => cy.wrap(res)).then(() => cy.submitAnswers()).then((onSelfAssessmentPage) => {
...
const navigateToCheckAnswersPage = (selectedQuestionsWithAnswers) => {
    return cy.navigateThroughQuestions(selectedQuestionsWithAnswers).then(() => cy.wrap(selectedQuestionsWithAnswers));
};
