...
public class RedisCacheTestsBase
{
    ...
    protected string Value = JsonSerialiser.Serialise(
        new ComponentInsetTextEntry { Text = "This is text" }
    );
    ...
    public RedisCacheTestsBase()
    {
        BackgroundTaskQueue
            .When(queue =>
                queue.QueueBackgroundWorkItemAsync(Arg.Any<Func<CancellationToken, Task>>())
            )
            .Do(task =>
            {
    ...
    protected virtual void MockDatabaseMethods()
    {
        ...
        Database
            .StringAppendAsync(Arg.Any<RedisKey>(), Arg.Any<RedisValue>(), Arg.Any<CommandFlags>())
            .Returns(
                (callinfo) =>
                {
                    var keyArg = callinfo.ArgAt<RedisKey>(0);
        ...
        Database
            .SetMembersAsync(Key, Arg.Any<CommandFlags>())
            .Returns(SetMembers.Select(member => new RedisValue(member)).ToArray());
        Database
            .SetAddAsync(Key, Arg.Any<RedisValue>(), Arg.Any<CommandFlags>())
            .Returns(
                (callinfo) =>
                {
                    var valueArg = callinfo.ArgAt<RedisValue>(1);
        ...
        Database
            .SetRemoveAsync(Key, Arg.Any<RedisValue>(), Arg.Any<CommandFlags>())
            .Returns(
                (callinfo) =>
                {
                    var valueArg = callinfo.ArgAt<RedisValue>(1);
