...
{
    private const string bodyJsonStr =
    ...
    private readonly IOptions<MessageRetryHandlingOptions> _options =
        Microsoft.Extensions.Options.Options.Create(new MessageRetryHandlingOptions());
    ...
    public async Task MessageRetryHandler_Should_Retry_Message_If_MaxRetries_Are_Not_Exhausted()
    {
        ...
        var serviceBusReceivedMessage = ServiceBusReceivedMessage.FromAmqpMessage(
            serviceBusMessage.GetRawAmqpMessage(),
            BinaryData.FromBytes(Encoding.UTF8.GetBytes(serviceBusReceivedMessageMock.LockToken))
        );

        var retryRequired = await _messageRetryHandler.RetryRequired(
        ...
        await _serviceBusSender
            .Received(1)
            .SendMessageAsync(Arg.Any<ServiceBusMessage>(), TestContext.Current.CancellationToken);
