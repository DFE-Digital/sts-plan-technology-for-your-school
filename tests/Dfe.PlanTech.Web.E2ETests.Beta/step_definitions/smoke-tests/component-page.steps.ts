...
Then('I should see a rich text paragraph with text {string}', async function (text: string) {
...
Then('I should see headings with the following texts:', async function (dataTable: DataTable) {
  const expectedHeadings = dataTable.raw().flat();
  for (const text of expectedHeadings) {
    const heading = this.page.locator('h1, h2, h3, h4, h5, h6').filter({ hasText: text });
    await expect(heading, `Heading not found: ${text}`).toBeVisible();
  }
});

Then('I should see an unordered list with items:', async function (dataTable: DataTable) {
  const items = dataTable.raw().flat();

  const ul = this.page.locator('main ul').filter({ hasText: items[0] }).first();
...
Then('I should see an ordered list with items:', async function (dataTable: DataTable) {
  const items = dataTable.raw().flat();
...
Then('I should see a table with column headings:', async function (dataTable: DataTable) {
  const headers = dataTable.raw().flat();
  const table = this.page.locator('table.govuk-table').first();
  await expect(table).toBeVisible();

  for (const headerText of headers) {
    const headerCell = table.locator('thead .govuk-table__header').filter({ hasText: headerText });
    await expect(headerCell, `Missing table header: ${headerText}`).toBeVisible();
  }
});

Then('I should see a table row with values:', async function (dataTable: DataTable) {
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    if (
      trimmedCells.length === rowValues.length &&
...
Then('the notification banner should contain bold text {string}', async function (text: string) {
  const banner = this.page.locator('.govuk-notification-banner');

  const bold = banner.locator('[class*="govuk-!-font-weight-bold"]', { hasText: text });

  await expect(bold, `Bold text not found in notification banner: ${text}`).toBeVisible();
});

Then('I should see inset text {string}', async function (text: string) {
  const inset = this.page.locator('.govuk-inset-text');
  await expect(inset).toBeVisible();
  await expect(inset).toHaveText(text);
});

Then('I should see an attachment titled {string}', async function (title: string) {
...
Then(
  ...
  async function (fileType: string, fileSize: string) {
    const metadata = this.page.locator('.attachment .attachment-metadata').first(); // first metadata block
...
Then('the attachment should have last updated text {string}', async function (lastUpdated: string) {
  const lastUpdatedMeta = this.page.locator('.attachment .attachment-metadata').nth(1); // second metadata block
...
function getAccordionSection(page: any, heading: string) {
  ...
  const section = accordion.locator('.govuk-accordion__section').filter({ hasText: heading });
...
When('I expand the accordion section {string}', async function (heading: string) {
  const section = getAccordionSection(this.page, heading);

  await expect(section, `Accordion section not found: ${heading}`).toHaveCount(1);
...
Then(
  ...
  async function (heading: string, text: string) {
    ...
    const bold = content.locator('[class*="govuk-!-font-weight-bold"]', { hasText: text });

    await expect(bold, `Bold text not found in section "${heading}": ${text}`).toBeVisible();
  },
);
...
Then(
  ...
  async function (heading: string, linkText: string, href: string) {
    ...
    await expect(link, `Link "${linkText}" not found in section "${heading}"`).toBeVisible();
    await expect(link).toHaveAttribute('href', href);
  },
);
...
Then(
  ...
  async function (heading: string, dataTable: DataTable) {
    ...
    const ul = content.locator('ul').filter({ hasText: items[0] }).first();
    ...
    for (const item of items) {
      const li = ul.locator('li', { hasText: item });
      await expect(li, `List item "${item}" not found in section "${heading}"`).toBeVisible();
    }
  },
);
...
Then(
  ...
  async function (heading: string, dataTable: DataTable) {
    ...
    const ol = content.locator('ol').filter({ hasText: items[0] }).first();
