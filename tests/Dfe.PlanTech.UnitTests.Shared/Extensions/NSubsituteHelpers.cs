...
{
    public static void HadMethodCalled<TSubstitute>(
        this TSubstitute substitute,
        string methodName,
        IEnumerable<object>? expectedArguments = null,
        int timesCalled = 1
    )
        where TSubstitute : class
    {
        ...
        foreach (var methodCall in methodCalls)
        {
            ...
            var missingArguments = expectedArguments
                .Where(expectedArg => !arguments.Any(arg => expectedArg.Equals(arg)))
                .ToArray();
    ...
    private static string GetMethodCallName(LambdaExpression expression)
    {
        ...
        var constant =
            methodCall.Object as ConstantExpression ?? throw new Exception("Object is null");
        var memberInfo =
            constant!.Value as MemberInfo ?? throw new Exception("Method does not exist");
