...
public static class AttachmentComponentRenderer
{
    public static StringBuilder AddHtml(RichTextContentField content, StringBuilder stringBuilder)
    {
        ...
        stringBuilder.Append(
            $"<a href=\"{customAttachment.Uri}\" aria-describedby=\"file-details\" class=\"govuk-link attachment-link\" download>{customAttachment.Title}"
        );
        stringBuilder.Append("</a></h2>");

        stringBuilder.Append("<p class=\"attachment-metadata\" id=\"file-details\">");
        stringBuilder.Append(
            $"<span class=\"attachment-attribute\" aria-label=\"file type\">{customAttachment.FileExtension!.ToUpper()}</span>,"
        );
    ...
    private static string GetImageTag(string fileExtension)
    {
        ...
        return fileExtension switch
        {
            ...
            FileExtensionConstants.CSV
            or FileExtensionConstants.XLS
            or FileExtensionConstants.XLSX => table,
