...
/// <summary>
/// Various extensions methods for repeating various operations against a DbContext using the <see cref="IDatabaseHelper{TDbEntity?}"/> class
/// </summary>
public static class IDatabaseHelperExtensions
{
    ...
    /// <param name="entity">The entity to match.</param>
    /// <returns>An <see cref="IQueryable{TDbEntity?}"/> of entities matching the specified entity.</returns>
    private static IQueryable<TDbEntity?> EntitiesMatchingId<TDbEntity, TDbContext>(this IDatabaseHelper<TDbContext> databaseHelper, TDbEntity entity)
      where TDbEntity : ContentComponentDbEntity
      where TDbContext : IDbContext
        => databaseHelper.GetQueryableForEntityExcludingAutoIncludesAndFilters(entity).Where(entity => entity.Id == entity.Id);
}
