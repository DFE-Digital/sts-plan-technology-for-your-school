...
public class RecommendationsRepository(ICmsDbContext db, ILogger<IRecommendationsRepository> logger)
    : IRecommendationsRepository
{
    private readonly ICmsDbContext _db = db;

    public async Task<SubtopicRecommendationDbEntity?> GetCompleteRecommendationsForSubtopic(string subtopicId,
        CancellationToken cancellationToken)
    {
        ...
        var intros = await _db.ToListAsync(
            _db.RecommendationIntros.Where(intro =>
                    intro.SubtopicRecommendations.Any(subtopicRec => subtopicRec.Id == recommendation.Id))
                .Include(intro => intro.Header)
            , cancellationToken);

        var chunks = await _db.ToListAsync(
            _db.RecommendationChunks.Where(chunk =>
                    chunk.RecommendationSections.Any(section => section.Id == recommendation.SectionId))
        ...
        var chunkContent = await _db.ToListAsync(
            _db.RecommendationChunkContents.Where(chunkContent => chunkContent.RecommendationChunk != null &&
                                                                  chunkContent.RecommendationChunk
                                                                      .RecommendationSections.Any(section =>
        ...
        return new SubtopicRecommendationDbEntity()
        {
            ...
            Intros = intros.Select(intro => new RecommendationIntroDbEntity()
            {
                ...
                Content =
                [
                    .. introContent.Where(content =>
                            content.RecommendationIntroId == intro.Id && content.ContentComponent != null)
            ...
            Section = new RecommendationSectionDbEntity()
            {
                Chunks = chunks.Select(chunk => new RecommendationChunkDbEntity()
                {
                    ...
                    Content =
                    [
                        .. chunkContent.Where(content =>
                                content.RecommendationChunkId == chunk.Id && content.ContentComponent != null)
                            .Select(content => content.ContentComponent)
                            .OrderBy(content => content?.Order)
                    ],
                    CSLink = chunk.CSLink
    ...
    public Task<RecommendationsViewDto?> GetRecommenationsViewDtoForSubtopicAndMaturity(string subtopicId,
        ...
        => _db.FirstOrDefaultAsync(
            _db.SubtopicRecommendations.Where(subtopicRecommendation => subtopicRecommendation.SubtopicId == subtopicId)
                .Select(subtopicRecommendation =>
                    subtopicRecommendation.Intros.FirstOrDefault(intro => intro.Maturity == maturity))
        ...
        where TContentComponentJoin : class, IHasContentComponent
    {
        ...
        logger.LogError("{ContentJoinType} has {InvalidRowsCount} rows missing ContentComponentId: {ErroredRowIds}",
            typeof(TContentComponentJoin).Name, invalidJoins.Length, invalidJoins.Select(join => join.Id));
    }
}
