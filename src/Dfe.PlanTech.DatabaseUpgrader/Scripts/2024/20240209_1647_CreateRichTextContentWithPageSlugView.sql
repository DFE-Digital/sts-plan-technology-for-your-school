...
OR ALTER VIEW Contentful.[RichTextContentsBySlug] AS (
  ...
  FROM 
            ...
            (
              ...
              FROM 
                (
                  SELECT 
                    ISNULL(
                      [BeforeContentComponentId], [ContentComponentId]
                    ) AS ContentId, 
                    P.Slug 
                  FROM 
    ...
    ) AS RichTextContentParents OUTER APPLY (
      SELECT 
        * 
      FROM 
        [Contentful].[SelectAllRichTextContentForParentId](
          RichTextContentParents.RichTextId
        )
