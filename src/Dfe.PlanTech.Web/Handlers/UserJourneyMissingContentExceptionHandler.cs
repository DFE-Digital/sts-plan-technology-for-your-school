...
{
    private readonly ILogger<UserJourneyMissingContentExceptionHandler> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    public async Task<IActionResult> Handle(Controller controller, UserJourneyMissingContentException exception)
    {
        ...
        var establishmentId = _currentUser.EstablishmentId
            ?? throw new InvalidDataException($"Current user has no {nameof(currentUser.EstablishmentId)}");
