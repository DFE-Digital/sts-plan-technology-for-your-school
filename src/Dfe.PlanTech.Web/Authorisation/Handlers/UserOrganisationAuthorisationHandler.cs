...
) : AuthorizationHandler<UserOrganisationAuthorisationRequirement>
{
    ...
    )
    {
        ...
        if (
            userAuthorisationResult == null
            || !userAuthorisationResult.PageRequiresAuthorisation
            || userAuthorisationResult.CanViewPage
            || RequestIsSignoutUrl(httpContext)
        )
        {
    ...
    private static UserAuthorisationResult? TryGetUserAuthorisationResult(HttpContext context)
    {
        if (
            context.Items.TryGetValue(
                UserAuthorisationResult.HttpContextKey,
                out object? userAuthorisationResult
            ) && userAuthorisationResult is UserAuthorisationResult result
        )
        {
            return result;
        }
    ...
    private static bool RequestIsSignoutUrl(HttpContext context) =>
        context.Request.Path.HasValue && context.Request.Path.Value == "/auth/sign-out";
}
