    ...
    : IRedisDependencyManager
{
    ...
    /// <inheritdoc cref="IRedisDependencyManager"/>
    public Task RegisterDependenciesAsync<T>(
        IDatabase database,
        string key,
        T value,
        ...
        backgroundTaskQueue.QueueBackgroundWorkItemAsync(
            (cancellationToken) => GetAndSetDependencies(database, key, value)
        );
    ...
    private async Task GetAndSetDependencies<T>(IDatabase database, string key, T value)
    {
        ...
        var tasks = GetDependencies(value)
            .Distinct()
            .Select(dependency =>
                batch.SetAddAsync(GetDependencyKey(dependency), key, CommandFlags.FireAndForget)
            )
            .ToArray();
        ...
        if (tasks.Length == 0)
        {
            ...
            tasks = tasks
                .Append(
                    batch.SetAddAsync(EmptyCollectionDependencyKey, key, CommandFlags.FireAndForget)
                )
                .ToArray();
        }
    ...
    private IEnumerable<string> GetDependencies<T>(T? value) =>
        value switch
        {
            ...
            _ => throw new InvalidOperationException(
                $"{value!.GetType()} is not a {typeof(ContentfulEntry)} or a {typeof(IEnumerable<ContentfulEntry>)}"
            ),
        };
