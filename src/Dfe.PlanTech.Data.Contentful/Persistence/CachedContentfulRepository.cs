...
public class CachedContentfulRepository(
    [FromKeyedServices(KeyedServiceConstants.ContentfulRepository)]
...
) : IContentfulRepository
{
    ...
    private readonly ICmsCache _cmsCache =
        cmsCache ?? throw new ArgumentNullException(nameof(cmsCache));
    ...
    public async Task<IEnumerable<TEntry>> GetEntriesAsync<TEntry>()
    {
        ...
        return await _cmsCache.GetOrCreateAsync(
                key,
                async () => await _contentfulRepository.GetEntriesAsync<TEntry>()
            ) ?? [];
    }
    ...
    public async Task<IEnumerable<TEntry>> GetEntriesAsync<TEntry>(GetEntriesOptions options)
    {
        ...
        return await _cmsCache.GetOrCreateAsync(
                key,
                async () => await _contentfulRepository.GetEntriesAsync<TEntry>(options)
            ) ?? [];
    }
