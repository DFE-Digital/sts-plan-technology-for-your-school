...
public class JsonToEntityMappers(IEnumerable<BaseJsonToDbMapper> mappers, JsonSerializerOptions jsonSerialiserOptions)
{
    private readonly JsonSerializerOptions _jsonSerialiserOptions = jsonSerialiserOptions;
    private readonly HashSet<BaseJsonToDbMapper> _mappers = mappers.ToHashSet();
    ...
    public Task<MappedEntity> ToEntity(string requestBody, CmsEvent cmsEvent, CancellationToken cancellationToken)
    {
        ...
        BaseJsonToDbMapper mapper = GetMapperForPayload(payload);

        return mapper.MapEntity(payload, cmsEvent, cancellationToken);
    }

    private BaseJsonToDbMapper GetMapperForPayload(CmsWebHookPayload payload)
