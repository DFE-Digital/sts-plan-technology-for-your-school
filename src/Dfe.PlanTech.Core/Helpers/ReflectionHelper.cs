...
public static class ReflectionHelper
{
    public static IEnumerable<Type> GetTypesInheritingFrom<TBase>() =>
        GetTypesInheritingFrom(typeof(TBase));

    public static IEnumerable<Type> GetTypesInheritingFrom(
        Type baseType,
        bool internalProjectsOnly = true
    ) =>
        AppDomain
    ...
    private static Func<Type, bool> InheritsBaseType(Type baseType) =>
        (type) => baseType.IsAssignableFrom(type) && type != baseType;
    ...
    private static bool IsNotTestProject(Assembly assembly, bool internalProjectsOnly) =>
        ...
        && (!internalProjectsOnly || assembly.FullName.Contains("Dfe.PlanTech."));

    public static bool HasParameterlessConstructor(this Type type) =>
        type.GetConstructor(Type.EmptyTypes) != null || type.GetConstructors().Length == 0;
