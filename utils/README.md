...
- **`update-gias-data`** — runs automatically via GitHub Actions on a daily schedule (`update-gias-data-scheduled.yml`). Run manually if the scheduled job fails or if you need an out-of-cycle refresh. Fetches establishment, group, and link data from the public EDUBASE API and updates the database with validation to catch data issues.

## See also

- [GitHub Actions workflows](../.github/README.md) — `update-gias-data-scheduled.yml` runs the GIAS update automatically
- [SQL data layer](../src/Dfe.PlanTech.Data.Sql/README.md) — the database tables these utilities interact with
