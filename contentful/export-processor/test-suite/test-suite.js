...
2 - Navigate ahead of journey via URL`;
  ...
  generateTestForMaturity(maturity) {
    ...
    const content = this.subtopic.recommendation.getContentForMaturityAndPath({
      maturity,
      path: pathForLow,
    });
  ...
  generateNavigateForRecommendationChunks(maturity) {
    ...
    const testSteps = [
      ...
      ...filteredChunks.map(
        (chunk, index) => `6.${index + 1} - Navigate to the '${chunk.header}' chunk`,
      ),
    ]
      .join('\n')
      .replace(',', '');
  ...
  generateCSLinkChunks(maturity) {
    ...
    if (filteredChunks.length === 0) {
      log(`No chunks with CSLink for maturity '${maturity}' in ${this.subtopicName}.`, {
        level: 'WARNING',
      });
      return;
    }
  ...
  getAnswerBasedChunks(chunks, answerIds) {
    return chunks.filter((chunk) => chunk.answers.some((answer) => answerIds.includes(answer.id)));
  }
