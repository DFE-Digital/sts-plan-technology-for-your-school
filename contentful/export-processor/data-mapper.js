  ...
  *sectionsToClasses(sections) {
  ...
  convertToMinimalSectionInfo(section, writeAllPossiblePaths = false) {
    return {
      ...
      allPathsStats: section.stats.pathsPerMaturity,
      minimumQuestionPaths: section.pathInfo.minimumPathsToNavigateQuestions,
      minimumRecommendationPaths: section.pathInfo.minimumPathsForRecommendations,
      allPossiblePaths: writeAllPossiblePaths
        ? section.pathInfo.paths.map((path) => {
            var result = {
              recommendation:
  ...
  mapRelationshipsForEntry(entry, contentType) {
    Object.entries(entry.fields).forEach(([key, value]) => {
      const referencedTypesForField = contentType.getReferencedTypesForField(key);
      ...
      if (Array.isArray(value)) {
        ...
        entry.fields[key] = this.getMatchingContentForReference(referencedTypesForField, value);
...
function logMissingContent(referencedTypesForField, item, entry) {
  ErrorLogger.addError({
    ...
    message: `Could not find matching content for ${referencedTypesForField} ${
      item?.sys?.id ?? item
    }`,
  });
}
