...
const createRowsFromSections = (sections) => {
  ...
  const getRowValuesForAnswer = (section, answer, chunk) => {
    ...
    return [section.name, matchingQuestion.text, answer.text, chunk.header, hyperlink];
  };
  ...
  return sections.flatMap((section) => {
    ...
    const createRow = ({ answer, chunk }) => getRowValuesForAnswer(section, answer, chunk);
...
const Csv = () => {
  const csvHeaders = ['Topic', 'Question', 'Answer', 'Outcome', 'Staging (Preview) URL'];
...
const main = async () => {
  ...
  const rows = createRowsFromSections(dataMapper.mappedSections).map(csv.joinRow);
