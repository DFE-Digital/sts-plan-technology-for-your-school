...
export class MinimumPathCalculator {
  ...
  calculateMinimumPathsToQuestions() {
    const remainingQuestions = this.questions.map((question) => question.id);
    const minimumPaths = [];
  ...
  handleRemainingQuestions(remainingQuestions, minimumPaths) {
    for (const questionId of remainingQuestions) {
      const pathContainingQuestion = this.getFirstPathContainingQuestion(questionId);
  ...
  _getPathForMaturity(maturity, minimumPathsForRecommendations) {
    const pathForRecommendation = this.sortedPaths.find((path) => path.maturity == maturity);
  ...
  removeItemsFromArray(array, itemsToRemove) {
    array.splice(0, array.length, ...array.filter((item) => !itemsToRemove.includes(item)));
  }
}
