...
export class AnswerPathCalculator {
    ...
    getPathsForAllAnswers(minimumPathsForRecommendations) {
        ...
        const answersUsed = this._getUsedAnswerIds(
        ...
        while (!this._allAnswersUsed(allAnswers, answersUsed)) {
            const startIndex =
                this._findLastQuestionWithUnusedAnswers(answersUsed);
    ...
    _initializePathStart(question, answersUsed) {
        const lastUnusedAnswers = question.answers.filter(
            (answer) => !answersUsed.includes(answer.id)
        );
        const latestAnswer = this.assignBestAnswer(
            lastUnusedAnswers,
    ...
    _buildBackwardsPath(path, startIndex, nextNeeded, answersUsed) {
        for (let i = startIndex - 1; i >= 0; i--) {
            ...
            const answer = this._findBestPreviousAnswer(
                currentQuestion,
                nextNeeded,
                answersUsed
            );
    ...
    _findBestPreviousAnswer(question, nextNeeded, answersUsed) {
        const unusedAnswers = question.answers.filter(
            (answer) =>
                !answersUsed.includes(answer.id) && !!answer.nextQuestion
        );

        if (
            unusedAnswers.some(
                (answer) => answer.nextQuestion.id === nextNeeded
            )
        ) {
            return unusedAnswers.find(
                (answer) => answer.nextQuestion.id === nextNeeded
            );
        }
    ...
    _buildForwardsPath(path, latestAnswer) {
        ...
        while (currentAnswer?.nextQuestion) {
            ...
            const nextAnswer = this.assignBestAnswer(
