...
export class PathCalculator {
  ...
  constructor({ questions, recommendation, sectionId }) {
    ...
    this.answerPathCalculator = new AnswerPathCalculator(questions, this.pathBuilder);
  ...
  get sortedPaths() {
    if (this._sortedPaths === undefined) {
      this._sortedPaths = this.paths.slice().sort((a, b) => b.path.length - a.path.length);
    }
    ...
    const uniqueTestedChunks = [
      ...new Set(
        [
          ...
          ...this.pathsForAllPossibleAnswers.map((userJourney) => userJourney.path),
        ]
    ...
    if (sectionChunks.length !== uniqueTestedChunks.length) {
      sectionChunks
        ...
        .forEach((missingChunk) =>
          this._addError(`Recommendation chunk ${missingChunk} not included in any test paths.`),
        );
    }
  }
  ...
  setNextQuestions() {
    for (const question of this.questions) {
      for (const answer of question.answers) {
        ...
        const matchingQuestions = this.questions.filter((q) => q.id == nextQuestionId);
