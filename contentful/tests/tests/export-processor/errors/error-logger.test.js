...
describe('ErrorLogger', () => {
  ...
  describe('addError', () => {
    ...
    it('should log error to console', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('article error for content 123: Missing title');
    });
  });
...
| 789 | Missing duration |`;
    ...
    it('should use custom file path when provided', () => {
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith('custom-path.md', expect.any(String));
    });

    it('should handle empty errors array', () => {
      errorLogger.writeErrorsToFile();

      expect(fs.writeFileSync).toHaveBeenCalledWith('content-errors.md', '# Content Errors: \n\n');
    });
  });
