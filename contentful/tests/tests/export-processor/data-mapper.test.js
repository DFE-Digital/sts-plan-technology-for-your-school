...
const DataMapper = (await import('../../../export-processor/data-mapper.js')).default;
...
describe('DataMapper', () => {
  ...
  describe('mapRelationshipsForEntry', () => {
    it('should map single reference relationships', () => {
      ...
      const mockContentType = {
        getReferencedTypesForField: jest.fn().mockReturnValue(['referencedType']),
      };
      const mockReferencedEntry = {
        fields: { name: 'Referenced Entry' },
      };

      mapper.contents.set('referencedType', new Map([['ref-id', mockReferencedEntry]]));
    ...
    it('should map array reference relationships', () => {
      ...
      const entry = {
        fields: {
          references: [{ sys: { id: 'ref-id-1' } }, { sys: { id: 'ref-id-2' } }],
        },
      };
    ...
    it('should log missing content for single reference relationships', () => {
      ...
      expect(errorLoggerSpy).toHaveBeenCalledWith({
        ...
        message: 'Could not find matching content for referencedType reference',
      });
    ...
    it('should log missing content for array reference relationships', () => {
      ...
      const entry = {
        ...
        fields: {
          references: [{ sys: { id: 'existing-ref' } }, { sys: { id: 'non-existent-ref' } }],
        },
      };

      mapper.contents.set(
        'referencedType',
        new Map([['existing-ref', { fields: { name: 'Existing Entry' } }]]),
      );
      ...
      expect(errorLoggerSpy).toHaveBeenCalledWith({
        id: 'test-entry-id',
        contentType: 'testType',
        message: 'Could not find matching content for referencedType non-existent-ref',
      });

      expect(entry.fields.references).toHaveLength(1);
      expect(entry.fields.references[0].fields.name).toBe('Existing Entry');
  ...
  describe('getMatchingContentForReference', () => {
    ...
    it('should return matching content for valid reference', () => {
      ...
      mapper.contents.set('testType', new Map([['content-id', mockContent]]));
  ...
  describe('mappedSections', () => {
    ...
    it('should map sections if not already cached', () => {
      ...
      expect(mapper.sectionsToClasses).toHaveBeenCalledWith(mapper.contents.get('section'));
  ...
  describe('sectionsToClasses', () => {
    ...
    const subtopicRecommendations = new Map([['rec1', mockSubtopicRecommendation]]);
    ...
    it('should return empty iterator when sections is null', () => {
      mapper.contents.set('subtopicRecommendation', subtopicRecommendations);
  ...
  describe('copyRelationships', () => {
    ...
    it('should map parent items to their corresponding children', () => {
      ...
      const parentArray = [{ sys: { id: 'child1' } }, { sys: { id: 'child2' } }];
    ...
    it("should return undefined for parent items that don't exist in children map", () => {
      ...
      const parentArray = [{ sys: { id: 'child1' } }, { sys: { id: 'nonexistent' } }];
