...
jest.mock('../../../../export-processor/user-journey/minimum-path-calculator.js');
jest.mock('../../../../export-processor/user-journey/answer-path-calculator.js');
...
describe('PathCalculator', () => {
  ...
  beforeEach(() => {
    ...
    PathBuilder.prototype.getPathsForQuestion = jest.fn().mockReturnValue(mockPaths);
    ...
    MinimumPathCalculator.prototype.calculateMinimumPathsForRecommendations = jest
      .fn()
      .mockReturnValue(new Map([['rec1', 'path1']]));
  ...
  describe('constructor', () => {
    it('should initialize with questions and recommendation', () => {
      ...
      expect(PathBuilder.prototype.getPathsForQuestion).toHaveBeenCalledWith(mockQuestions[0]);
    });

    it('should create user journeys from paths', () => {
      expect(pathCalculator.paths.length).toBe(mockPaths.length);
      expect(UserJourney.prototype.setRecommendation).toHaveBeenCalledWith(mockRecommendation);
    });
  });
  ...
  describe('minimumPathsToNavigateQuestions', () => {
    it('should return minimum paths to navigate questions', () => {
      ...
      expect(MinimumPathCalculator.prototype.calculateMinimumPathsToQuestions).toHaveBeenCalled();
    });
  ...
  describe('pathsForAllPossibleAnswers', () => {
    it('should return paths for all possible answers', () => {
      ...
      expect(AnswerPathCalculator.prototype.getPathsForAllAnswers).toHaveBeenCalled();
    });
    ...
    it('should return cached paths for all possible answers', () => {
      ...
      expect(AnswerPathCalculator.prototype.getPathsForAllAnswers).not.toHaveBeenCalled();
    });
  });
  ...
  describe('checkAllChunksTested', () => {
    ...
    it('should correctly map chunk IDs', () => {
      ...
      mockRecommendation.section.getChunksForPath.mockReturnValue(mockChunks);

      const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => {});

      pathCalculator.checkAllChunksTested();

      expect(mockRecommendation.section.chunks.map((chunk) => chunk.id)).toEqual([
        'chunk1',
        'chunk2',
      ]);
