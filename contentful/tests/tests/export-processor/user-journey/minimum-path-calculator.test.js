...
describe('MinimumPathCalculator', () => {
  ...
  beforeEach(() => {
    ...
    mockSortedPaths = [...mockPaths].sort((a, b) => b.path.length - a.path.length);
  ...
  describe('getUniquePaths', () => {
    it('should return unique paths based on questions answered', () => {
      const duplicatePaths = [
        ...mockSortedPaths,
        createMockUserJourney({
          path: [{ question: mockQuestions[0], answer: { id: 'a6' } }],
          maturity: 'Low',
        }),
      ];
      ...
      const questionPatterns = uniquePaths.map((p) => p.questionIdsAnswered.join(','));
  ...
  describe('calculateMinimumPathsToQuestions', () => {
    it('should find minimum paths to cover all questions', () => {
      ...
      const coveredQuestions = new Set(
        minimumPaths.flatMap((path) => path.map((part) => part.question.id)),
      );
    ...
    it('should handle questions with no available paths', () => {
      const questionsWithExtra = [...mockQuestions, { id: 'q4', text: 'Question 4' }];
  ...
  describe('calculateMinimumPathsForRecommendations', () => {
    it('should find paths for each maturity level', () => {
      const recommendationPaths = calculator.calculateMinimumPathsForRecommendations();
    ...
    it('should log error for missing maturity paths', () => {
      const pathsWithMissingMaturity = mockPaths.filter((p) => p.maturity !== 'Medium');
    ...
    it('should return paths that match maturity levels', () => {
      ...
      mockPaths.forEach((mockPath) => {
        if (mockPath.maturity) {
          expect(recommendationPaths[mockPath.maturity]).toBeDefined();
          expect(recommendationPaths[mockPath.maturity]).toEqual(mockPath.path);
        }
      });
    });
  });
  ...
  describe('handleRemainingQuestions', () => {
    it('should add paths for remaining questions when paths exist', () => {
      ...
      const initialPaths = [{ question: mockQuestions[0], answer: { id: 'a1' } }];
      ...
      const questionIds = result.flatMap((path) =>
        Array.isArray(path) ? path.map((part) => part.question.id) : [],
      );
