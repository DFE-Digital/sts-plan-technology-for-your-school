...
const contentfulImportMock = jest.fn().mockImplementation(() => Promise.resolve());
jest.unstable_mockModule('contentful-import', () => ({
  default: contentfulImportMock,
}));

const getAndValidateClientMock = jest.fn().mockImplementation(() => Promise.resolve(true));
const getClientMock = jest.fn().mockImplementation(() => ({
  getAndValidateClient: getAndValidateClientMock,
}));

jest.unstable_mockModule('../../../../content-management/helpers/get-client.js', getClientMock);

const contentfulImport = (await import('contentful-import')).default;
const existsSync = (await import('fs')).existsSync;

const getAndValidateClient = (await import('content-management/helpers/get-client.js'))
...
const importContentfulData = (await import('content-management/helpers/import-content.js')).default;
...
describe('importContentfulData', () => {
  ...
  it('should throw an error when content file does not exist', async () => {
    ...
    await expect(importContentfulData).rejects.toThrow(`File not found: ${mockEnv.CONTENT_FILE}`);
  });
  ...
  it('should handle contentful import errors', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith('Error during import:', mockError);
  });
