...
const getAndValidateClientMock = jest.fn().mockImplementation(() => Promise.resolve(mockClient));
...
const findAndDeleteMissingReferences = (
  await import('../../../../content-management/changes/20241202-1200-find-and-delete-missing-references.js')
).default;
...
describe('Find and Delete Missing References', () => {
  ...
  test('should remove invalid references from single reference fields', async () => {
    ...
    expect(mockClient.entry.update).toHaveBeenCalledWith({ entryId: mockEntry.sys.id }, mockEntry);
  });
  ...
  test('should handle errors gracefully', async () => {
    ...
    await expect(findAndDeleteMissingReferences()).rejects.toThrow('API Error');
  });
});
