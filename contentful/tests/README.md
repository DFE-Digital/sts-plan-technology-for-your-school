...
Shared Jest test suite for the Node.js tools in the `contentful/` directory. Tests live here rather
than inside each individual tool to allow a single `npm test` command at the `contentful/` level to
run all of them.
...
1. Create `.vscode/launch.json` in the **repository root** (if it doesn't already exist) with the
   following configuration, adjusting the `args` path to the test file you want to debug:
     ...
     "configurations": [
       {
         ...
         "args": ["tests/tests/export-processor/content-types/section.test.js", "--runInBand"],
...
3. Open the **Run and Debug** panel in VS Code, select **Debug Jest Test**, and click the green play
   button.
4. Select 'Debug Jest Test' from the drop down menu.
